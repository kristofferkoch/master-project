...
When producing ICs, it is vital to ensure that the defective ICs are
...
performed. Testing methods was divided into to categories by
...
\approx 10^19$ vectors must be tested, which will, at an unreasonable
high test frequency of \unit{1}{\giga \hertz}, take approximatly
$10^10$ s, or 317 years to perform, leaving functional testing
unreasonable for this circuit.
...
While it is argumented in \cite[pp xxx]{sparso} that a clockless circuit
has a high probability of deadlock in the presence of a fault, it is
illustrated in \cite[pp xx]{fullscan} how a fault can also make a
circuit malfunction. Furthermore, in \cite{fullscan}, methods for
