...
The performance of the openssl AES CBC implementation\footnote{openssl
...
above, have some negative effect on the performance, as there will be
a greater control overhead. The area will also most likely increase by
a factor, but as most of the operations can be generalized to also
work in inverse, the overhead should not be dramatic. Expansion of the
supported key sizes, in addition to the obvious demand of larger
registers, only requires minor expansion of the control logic.
